...
export function MessagePortal() {
  ...
  // Live-Schimpfwortprüfung über alle Textfelder.
  ...
  // Verlauf + Entwurf aus localStorage laden.
  useEffect(() => {
  ...
  // Entwurf automatisch speichern (entprellt).
  useEffect(() => {
  ...
  function validate() {
    ...
    if (!subject.trim()) next.subject = "Bitte gib einen Betreff ein."
    if (message.trim().length < 10)
      next.message = "Deine Nachricht sollte mindestens 10 Zeichen lang sein."
    if (!consent) next.consent = "Bitte bestätige vor dem Senden."
    setErrors(next)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Schimpfwortfilter blockiert das Senden.
    if (hasProfanity) {
      triggerShake()
      showToast(
        "error",
        "Bitte entferne unangemessene Ausdrücke, bevor du sendest.",
      )
      return
    }
    ...
    try {
      ...
      if (!res.ok) {
        showToast(
          "error",
          data.error ?? "Etwas ist schiefgelaufen. Bitte erneut versuchen.",
        )
        triggerShake()
        return
      }
      ...
      showToast("error", "Netzwerkproblem. Bitte prüfe deine Verbindung.")
      triggerShake()
    } finally {
      setSubmitting(false)
    }
        ...
        >
          <header className="flex flex-col items-center text-center">
            ...
            <p className="mt-3 max-w-sm text-pretty text-sm leading-relaxed text-muted-foreground">
              Ein ruhiger, privater Weg, mich zu erreichen. Ich lese jedes Wort
              und antworte so bald ich kann.
            </p>
          </header>
          ...
          <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-6">
            ...
            <Field index={0} label="Dein Name" error={errors.name}>
              <input
                type="text"
                value={name}
                maxLength={80}
            ...
            <Field index={2} label="Betreff" error={errors.subject}>
              <input
                type="text"
                value={subject}
                maxLength={120}
            ...
            <Field index={3} label="Nachricht" error={errors.message}>
              <textarea
                value={message}
              ...
              {hasProfanity && (
                <div className="animate-rise mt-2 flex items-start gap-2 rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-2.5 text-xs leading-relaxed text-destructive">
                  ...
                  <span>
                    Bitte achte auf einen respektvollen Ausdruck. Unangemessene
                    Wörter müssen vor dem Senden entfernt werden.
                  </span>
                </div>
              )}
            </Field>

            <Field index={4} label="Anhang" hint="optional, bis zu 8 MB">
              <FileDropzone
                files={files}
                onChange={setFiles}
            ...
            <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-border bg-background/40 p-4 transition-colors hover:border-primary/40">
              ...
              <span className="text-xs leading-relaxed text-muted-foreground">
                Ich verstehe, dass mein Name, meine Nachricht
                {contact.trim() ? ", meine Kontaktangabe" : ""} und etwaige
                Anhänge an den Empfänger gesendet werden, um mein Anliegen zu
                bearbeiten. Es werden keine Standort-, IP- oder Gerätedaten
                erfasst.
              </span>
            </label>
            {errors.consent && (
            ...
            <p className="flex items-center justify-center gap-1.5 text-center text-xs text-muted-foreground">
              ...
              Sicher zugestellt. Nur das, was du schreibst, wird geteilt.
            </p>
          </form>
        </section>
      ...
      >
        ...
        >
          <span className="mx-auto flex size-16 items-center justify-center rounded-full border border-primary/40 bg-primary/15">
            {success && <Check className="animate-pop size-8 text-primary-foreground" />}
          </span>
          <h2 className="mt-5 font-serif text-2xl font-semibold text-foreground">
            Nachricht gesendet
          </h2>
          <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
            Deine Nachricht ist sicher angekommen. Ich melde mich bald.
          </p>
          <button
            type="button"
