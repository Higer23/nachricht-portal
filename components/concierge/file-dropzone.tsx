...
type FileDropzoneProps = {
  ...
  dict: Dictionary["dropzone"]
}

export function FileDropzone({ files, onChange, onError, dict }: FileDropzoneProps) {
  ...
  function addFiles(incoming: File[]) {
    ...
    for (const file of incoming) {
      if (file.size > MAX_FILE_SIZE) {
        // e.g. "„foto.jpg" ist größer als 8 MB." / "„foto.jpg" MB'den büyük."
        onError(`„${file.name}" ${dict.tooBig}`)
        continue
      }
      if (next.length >= MAX_FILES) {
        onError(dict.tooMany)
        break
      }
      next.push(file)
    }
    ...
    <div className="flex flex-col gap-3">
      ...
      >
        ...
        <span className="flex flex-col">
          <span className="text-sm font-medium text-foreground">{dict.heading}</span>
          <span className="text-xs text-muted-foreground">{dict.hint}</span>
        </span>
      </button>
        ...
        <ul className="flex flex-col gap-2">
          {files.map((file, index) => (
            ...
            >
              ...
              <button
                type="button"
                aria-label={`${file.name} ${dict.removeLabel}`}
